...
//Select API
app.post('/select/(:ugtypeId)', (req, res) => {
  let token = req.headers.authorization;
  if (token) {
    auth.doDecodeToken(token).then(async (ur) => {
      if (ur.length > 0){
        try {
          const ugtypeId = req.params.ugtypeId;
          const result = await Urgenttype.findAll({attributes: excludeColumn, where: {id: ugtypeId}});
          res.json({status: {code: 202}, Records: result});
        } catch(error) {
          log.error(error);
          res.json({status: {code: 500}, error: error});
        }
  ...
  if (token) {
    auth.doDecodeToken(token).then(async (ur) => {
      if (ur.length > 0){
        ...
        res.json({status: {code: 200}, Record: adUrgenttype});
      } else {
        log.info('Can not found user from token.');
        res.json({status: {code: 203}, error: 'Your token lost.'});
      }
  ...
  if (token) {
    auth.doDecodeToken(token).then(async (ur) => {
      if (ur.length > 0){
        ...
        let updateUrgenttype = req.body.data;
